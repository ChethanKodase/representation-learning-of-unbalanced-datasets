...
test_majority_class_index = 1 # the class of which you want the reconstructions
...
for test_majority_class_index in class_labels:
    ...
    for test_data_noise_percent in [0.0, 0.1, 0.2, 0.5, 0.7]:
        ...
        train_samples = train_batches.reshape(train_batches.shape[0]*train_batches.shape[1], no_channels, dx, dy).to(device)
        ...
        # Concatenate the first sample from test_samples with the train samples

        neighbourhood = torch.cat((test_samples[3].unsqueeze(0), train_samples), dim=0)

        print('neighbourhood.shape', neighbourhood.shape)

        # Common hyper parameters
        layer_size = 100
        latent_dim = 4
        for latent_dim in [4, 10]:
            no_layers = 3
            no_epochs = 100
            ...
            if(train_AE_MLP):
                ...
                embedding = model_mlpae.encoder(test_samples)
            ...
            if(train_AE_REG):
                ...
                embedding = model_aereg.encoder(test_samples)
            ...
            if(train_CNN_AE):
                ...
                embedding = model_cnnae.encoder(test_samples)
            ...
            if(train_Contra_AE):
                ...
                def model_contra(batch_x):
                    ...
                    return model_contra_.encoder(batch_x_in)

                embedding = model_contra(test_samples)
            ...
            if(train_MLPVAE):
                ...
                def model_mlpvae(batch_x):
                    batch_x_in = batch_x.reshape(-1, dx*dy).to(device)    
                    recon = model_mlpvae_.fc1(model_mlpvae_.encoder(batch_x_in))
                    return recon
                
                embedding = model_mlpvae(test_samples)
            ...
            if(train_CNN_VAE):
                ...
                def model_cnnvae(batch_x):
                    recon = model_cnnvae_.fc1(model_cnnvae_.encoder(batch_x))
                    return recon

                embedding = model_cnnvae(test_samples)
                #psnr_of_samples, ssim_of_samples =  get_batch_psnr_ssim_lists(test_samples, reconstructions_cnnvae)


            # print all the embeddings
            #print("embedding_mlpae.shape", embedding_mlpae.shape)
            #print("embedding_aereg.shape", embedding_aereg.shape)
            #print("embedding_cnnae.shape", embedding_cnnae.shape)
            #print("embedding_contra.shape", embedding_contra.shape)
            #print("embedding_mlpvae.shape", embedding_mlpvae.shape)
            #print("embedding_cnnvae.shape", embedding_mlpvae.shape)



            # find out the the k nearest neighbors of the first element of embedding_mlpae in the embedding_mlpae
            k = 100
            distances, indices = torch.topk(torch.norm(embedding[0]-embedding, dim=1), k=k, largest=False)
            #indices = indices[1:]
            ...
            for i in range(10):
                for j in range(10):
                    axs[i,j].imshow(test_samples[i*10+j][0].cpu().detach().numpy())
                    axs[i,j].axis('off')
            ...
            plt.savefig('./results_plotting/latent_neighbourhood/CNN_VAE/latent_dim'+str(latent_dim)+'/perturbation_level_'+str(int(test_data_noise_percent*100))+'_percent/class_'+str(classes[test_majority_class_index])+'_inputPerturbLevel_'+str(int(test_data_noise_percent*100))+'_recons.png')
